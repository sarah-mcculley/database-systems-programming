...
DECLARE 
	...
	select  StudentID, SUBSTR(CourseName, -1) AS lastDigit, grade, grade.sectionID
	from grade, section, course
	where grade.sectionID = section.sectionID and grade.CourseID = course.CourseID
	order by StudentID;
...
BEGIN 
	...
	LOOP
		FETCH grade_cursor
		INTO myStudentID, myLastDigit, myGrade, mySectionID;
		EXIT WHEN grade_cursor%NOTFOUND;

		IF myLastDigit < 'g' THEN
			update grade set grade = 'A' 
			where grade.sectionID = mysectionID and studentID = myStudentID;
		ELSIF myLastDigit >= 'g' and myLastDigit < 'l' THEN
			update grade set grade = 'B' 
			where grade.sectionID = mySectionID and studentID = myStudentID;
		ELSIF myLastDigit >= 'l' or myLastDigit < 'q' THEN
			update grade set grade = 'C' 
			where grade.sectionID = mySectionID and studentID = myStudentID;
		ELSIF myLastDigit >= 'q' or myLastDigit < 'u' THEN
			update grade set grade = 'D' 
			where grade.sectionID = mySectionID and studentID = myStudentID;
		ELSE
			update grade set grade = 'E'
			where grade.sectionID = mySectionID and studentID = myStudentID;
		END IF;
	END LOOP;
	CLOSE grade_cursor;
...
/* this view can be created, but I am having trouble solving casting problems to select from it */
CREATE or REPLACE VIEW StudentGrades AS
SELECT StudentCourses.StudentID, StudentCourses.FirstName, StudentCourses.LastName, StudentCourses.CourseID, 
StudentCourses.CourseName, StudentCourses.SectionID, InstructorCourses.FirstName AS FacultyFirst, 
InstructorCourses.LastName AS FacultyLast, CreditHours, Grade,
substr(TO_CHAR(InstructorCourses.StartTime, 'MM/DD/YYYY hh:mi AM'), 0, instr(TO_CHAR(InstructorCourses.StartTime, 'MM/DD/YYYY hh:mi AM'), ' ')-1) as StartDate, 
substr(TO_CHAR(InstructorCourses.StartTime, 'MM/DD/YYYY hh:mi AM'), instr(TO_CHAR(InstructorCourses.StartTime, 'MM/DD/YYYY hh:mi AM'), ' ')+1) as StartTime
from StudentCourses, InstructorCourses, grade
where StudentCourses.Days = InstructorCourses.Days
and StudentCourses.SectionID = grade.SectionID
and StudentCourses.StudentID = grade.StudentID
set linesize 1500;
select * from StudentGrades
order by studentID;
;

/* this view works to be selected from without startDate, StartTime, etc */
CREATE or REPLACE VIEW StudentGrades AS
SELECT StudentCourses.StudentID, StudentCourses.FirstName, StudentCourses.LastName, StudentCourses.CourseID, 
StudentCourses.CourseName, StudentCourses.SectionID, InstructorCourses.FirstName AS FacultyFirst, 
InstructorCourses.LastName AS FacultyLast, CreditHours, Grade
